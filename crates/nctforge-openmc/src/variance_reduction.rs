...
//! `nctforge.variance-reduction/0.1.0` spec into a concrete
//! `nctforge.weight-windows/0.1.0` artifact. `uniform` and `explicit`
...
pub const VR_VALIDATION_SCHEMA: &str = "nctforge.vr-validation/0.1.0";
/// Resolved weight-window artifact carried inside a generated deck.
pub const RESOLVED_WW_FILE: &str = "nctforge-weight-windows.json";
...
fn derive_forward_flux(
    window: &nctforge_transport::WeightWindowSpec,
...
pub fn validate_variance_reduction(
    ...
    if reference.schema_version != crate::acceptance::ACCEPTANCE_REPORT_SCHEMA {
    ...
    let manifest_bytes =
        std::fs::read(vr_run_directory.join(crate::statepoint::OPENMC_INPUT_MANIFEST_FILE))
...
pub enum VarianceReductionError {
    #[error("{0}")]
    ResolvedInvalid(#[from] nctforge_transport::VarianceReductionError),
...
mod tests {
    ...
    fn spec(bounds: WeightWindowBounds) -> VarianceReductionSpec {
        VarianceReductionSpec {
            schema_version: nctforge_transport::VARIANCE_REDUCTION_SCHEMA.into(),
    ...
    fn uniform_resolves_without_statepoint() {
        ...
        let resolved =
            resolve_weight_windows(&s, &"ab".repeat(32), "nctforge.test.ww.v1", None).unwrap();
    ...
    fn synthetic_statepoint() -> OpenMcStatepoint {
        ...
        let tally = crate::statepoint::OpenMcStatepointTally {
            id: 10,
            name: "nctforge.diagnostic.photon_fluence".into(),
