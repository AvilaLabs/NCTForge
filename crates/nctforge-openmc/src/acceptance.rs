...
//! `nctforge.openmc-acceptance-report/0.1.0` document.
//!
...
pub const ACCEPTANCE_CONTRACT_FILE: &str = "nctforge-acceptance-contract.json";
pub const ACCEPTANCE_REPORT_SCHEMA: &str = "nctforge.openmc-acceptance-report/0.1.0";
...
pub fn evaluate_runs(
    ...
    let acceptance: OpenMcAcceptanceContract = serde_json::from_slice(
        &std::fs::read(run_directories[0].join(ACCEPTANCE_CONTRACT_FILE))
    ...
    for run in &runs {
        for (roi_index, roi) in manifest.rois.iter().enumerate() {
            ...
            let name = |suffix: &str| format!("nctforge.roi.{}.{suffix}", roi.name);
...
fn load_run(directory: &Path, exit_code: i32) -> Result<RunData, OpenMcAcceptanceError> {
    ...
    let (manifest_bytes, manifest_sha256) = read("nctforge-input-manifest.json")?;
...
mod tests {
    ...
    fn region_tallies(
        ...
        let mut push = |slot: u32, suffix: &str, values: Vec<f64>| {
            out.push(TallySpec {
                id: id_base + slot,
                name: format!("nctforge.roi.{roi}.{suffix}"),
                bins,
                rel_sigma: REL_SIGMA,
                means: values,
            });
    ...
    fn scoring_tallies() -> Vec<TallySpec> {
        ...
        vec![
            mk(1, "nctforge.component.boron.response", 1.0e-12),
            mk(2, "nctforge.component.nitrogen.response", 2.0e-13),
            mk(3, "nctforge.component.hydrogen.response", 5.0e-14),
            mk(4, "nctforge.component.photon.heating", 2.0e3),
            mk(5, "nctforge.physical_total.coupled_heating", 9.8e3),
        ]
    }

    fn contract_json() -> serde_json::Value {
        serde_json::json!({
            "schema_version": "nctforge.acceptance-contract/0.1.0",
    ...
    fn write_run(directory: &Path, seed: u64, perturb_boron: f64) -> PathBuf {
        ...
        let tally_entries: Vec<serde_json::Value> = tallies
            .iter()
            .map(|spec| {
                let roi = if spec.name.contains("nctforge.roi.core") {
                    Some("core")
                } else if spec.name.contains("nctforge.roi.axis") {
                    Some("axis")
                } else {
                    None
                };
        ...
        let manifest = serde_json::json!({
            "schema_version": "nctforge.openmc-input-manifest/0.2.0",
            "case_id": "synthetic-case",
            "backend_id": "openmc",
    ...
    fn fails_when_seed_results_disagree() {
        ...
        let boron = report
            .chi_square
            .iter()
            .find(|c| c.tally == "nctforge.roi.core.component.boron.response")
            .unwrap();
        assert!(!boron.passed);
        assert!(!report.gates_passed);
    }
