...
use nctforge_core::{ContentReference, RegionMask, equivalent_uniform_dose, masked_values, mean};
use serde::{Deserialize, Serialize};
use sha2::{Digest, Sha256};

use crate::BioError;

pub const ENDPOINT_MODEL_SCHEMA: &str = "nctforge.endpoint-model/0.1.0";
pub const ENDPOINT_EVALUATION_SCHEMA: &str = "nctforge.endpoint-evaluation/0.1.0";
...
impl EndpointModel {
    pub fn validate(&self) -> Result<(), BioError> {
        if self.schema_version != ENDPOINT_MODEL_SCHEMA {
...
impl EndpointEvaluation {
    pub fn validate(&self) -> Result<(), BioError> {
        if self.schema_version != ENDPOINT_EVALUATION_SCHEMA {
            return Err(BioError::UnsupportedSchema(self.schema_version.clone()));
        }
...
mod tests {
    ...
    fn model(function: EndpointFunction) -> EndpointModel {
        ...
        EndpointModel {
            ...
            id: "nctforge.tests.endpoint.v1".into(),
