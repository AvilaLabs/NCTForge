...
//! Public conformance suite for `nctforge.endpoint-model/0.1.0` →
//! `nctforge.endpoint-evaluation/0.1.0` scoring and UTCP combination.
//!
...
//! with `NCTFORGE_UPDATE_CONFORMANCE=1 cargo test -p nctforge-bio`.

use std::fs;
use std::path::{Path, PathBuf};

use nctforge_bio::{BioError, EndpointEvaluation, EndpointModel, RegionMask, UtcpCombination};
...
fn endpoint_model_conformance_suite() {
    ...
    let update = std::env::var_os("NCTFORGE_UPDATE_CONFORMANCE").is_some();
    let mut ok_count = 0;
