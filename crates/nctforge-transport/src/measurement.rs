...
//! (`nctforge.measurement-record`, `nctforge.measurement-comparison`).
//!
...
use nctforge_core::{ContentReference, ValidationError};
...
pub const MEASUREMENT_RECORD_SCHEMA: &str = "nctforge.measurement-record/0.1.0";
pub const MEASUREMENT_COMPARISON_SCHEMA: &str = "nctforge.measurement-comparison/0.1.0";
...
impl MeasurementRecord {
    pub fn validate(&self) -> Result<(), MeasurementError> {
        if self.schema_version != MEASUREMENT_RECORD_SCHEMA {
...
impl MeasurementComparisonReport {
    pub fn validate(&self) -> Result<(), MeasurementError> {
        if self.schema_version != MEASUREMENT_COMPARISON_SCHEMA {
            return Err(MeasurementError::UnsupportedSchema(
                self.schema_version.clone(),
            ));
        }
