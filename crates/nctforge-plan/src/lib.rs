...
//! Tabular CSV/XLSX interchange for `nctforge.exposure-plan` documents.
//!
...
/// metadata is present.
pub const TABLE_FORMAT: &str = "nctforge.exposure-plan-table/1";
...
fn grid_to_plan(
    ...
    let plan = ExposurePlan {
        schema_version: nctforge_core::EXPOSURE_PLAN_SCHEMA.into(),
        id,
        case_id,
        covariance,
        exposures,
...
/// Run a saved `nctforge.exposure-plan/0.1.0` file end to end: verify every
...
pub fn accumulate_plan_file(plan_path: &Path) -> Result<PhysicalDoseBundle, PlanRunError> {
    ...
    Ok(nctforge_core::accumulate_exposures(
        &plan,
        &plan_sha256,
        &bundles,
    )?)
